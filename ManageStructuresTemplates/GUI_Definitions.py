...
gui_definition: {DataFrame} -- All information required to construct and
    ...
    appearance: {dict} -- Contains items describing the widget's appearance:
    ...
    geometry: {dict} -- The padding around the widget.
    ...
    FrameConfiguration:
        column_number: {int} -- The index to the grid column to be configured.
    	minsize: {int} -- The column or row's minimum size in pixels.
        pad: {int} -- A number of pixels that will be added to the given column
            or row, over and above the largest cell in the column or row.
        weight: {int} -- The relative weight of this column or row to be used
            when distributing the extra space while stretching. If this option
            is not used, the column or row will not stretch.
'''
